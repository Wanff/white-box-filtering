...
class TextMonitor(Monitor):
    def __init__(self, model : AutoModelForCausalLM | AutoModelForSequenceClassification, tokenizer : AutoTokenizer, 
        ...
        if config_name == "llamaguard":
            ...
            self.use_head = False
        elif config_name == "llama2-short-head":
            self.instruction_prompt = LlamaGuardPlusConfig['instruction_prompt']
            self.model_name = 'llama-2-7b-for-harm-classification'
            self.use_head = True
        else:
    ...
    def predict_proba(self, input_ids : torch.Tensor):
        ...
        if self.use_head:
            output = self.model(inputs_embeds=inputs_embeds,
                                past_key_values=kv_cache_batch if self.kv_cache is not None else None).logits
            proba = output.softmax(dim = -1)[:, 1]
            return proba
        else:
            output = self.model(inputs_embeds=inputs_embeds,
                                past_key_values = kv_cache_batch if self.kv_cache is not None else None)
    ...
    def _predict_proba_from_embeds(self, embeds: torch.Tensor): 
        ...
        if self.use_head:
            output = self.model(inputs_embeds=embeds,
                                past_key_values=self.kv_cache if self.kv_cache is not None else None).logits
            proba = output.softmax(dim = -1)[0, 1]
            return proba
        else:
            output = self.model(inputs_embeds=embeds,
    ...
    def get_batched_preds(self, prompts: List[str], batch_size: int = 8) -> np.ndarray:
        ...
        for i in tqdm(range(0, len(prompts), batch_size)):
            ...
            if self.use_head:
                preds.append(output.logits.softmax(-1)[:, 1].cpu().detach().numpy())
            else:
